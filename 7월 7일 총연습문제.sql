...
CREATE SEQUENCE CUSTOMER_SEQ
     ...
     CACHE 20
     ORDER;
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES (CUSTOMER_SEQ.NEXTVAL, '박지성', '영국', '000-000-000');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES (CUSTOMER_SEQ.NEXTVAL, '김연아', '대한민국', '111-111-111');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES (CUSTOMER_SEQ.NEXTVAL, '장미란', '대한민국', '222-222-222');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES (CUSTOMER_SEQ.NEXTVAL, '추신수', '미국', '333-333-333');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES (CUSTOMER_SEQ.NEXTVAL, '박세리', '대한민국', NULL);
COMMIT;
...
ALTER TABLE ORDER_T ADD CONSTRAINT PK_ORDER으로 PRIMARY KEY(ORDER_ID);
...
SELECT O.ORDER_ID AS 주문번호
 ...
 ORDER BY O.ORDER_DT ASC;                                              --ORDER_DT는 이미 INSERT부분에 값을 넣었음
...
SELECT C.CUST_NAME AS 고객명, O.CUST_ID
  FROM CUSTOMER_T C LEFT OUTER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
...
SELECT O.ORDER_ID AS 주문번호
     , B.BOOK_ID AS 책번호
     , B.BOOK_NAME AS 책이름
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O                                      --한쪽에 있는 정보만 가지고 올때는 외부조인
    ON B.BOOK_ID = O.BOOK_ID
 WHERE O.ORDER_ID IS NULL;
...
-- 김연아  2
SELECT C.CUST_NAME       AS 고객명
     , COUNT(O.ORDER_ID) AS 구매도서수                --주문횟수를 센다고 생각
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;

-- '김연아' 구매한 책이 몇 종인가? (피겨의 정석을 2번 주문했다면 구매한 책은 1종)
SELECT C.CUST_NAME       AS 고객명
     , COUNT(DISTINCT O.BOOK_ID) AS 구매도서수                --주문횟수를 센다고 생각
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;
...
SELECT A.CUST_NAME AS 고객명
  ...
  FROM (SELECT RANK() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS RK
          ...
          FROM CUSTOMER_T C INNER JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
            ON B.BOOK_ID = O.BOOK_ID
         GROUP BY C.CUST_ID, C.CUST_NAME) A                    --인라인 뷰 별명 A
