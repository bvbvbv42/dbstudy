...
-- 활용1. WITH문과 재귀 쿼리 //여기서부턴 몰라도됨
WITH
    MY_SUBQUERY(N, TOTAL) AS (                    -- N, TOAL은 MY_SUBQUERY의 칼럼을 의미한다.
        SELECT 1, 1                               -- N=1, TOTAL=1을 의미하는 초기화 서브쿼리
          FROM DUAL
         UNION ALL
        SELECT N + 1, TOTAL + (N + 1)             -- N = N + 1 , TOAL = TOTAL + (N + 1) 방식으로 반복해서 처리되는부분
          FROM MY_SUBQUERY
         WHERE N < 10
)
