...
SELECT *
  FROM  EMPLOYEES
  WHERE SALARY >= 10000;
--5. 사원 테이블에서 연봉이 10000 이상, 20000 이하의 사원의 정보 조회하기
SELECT *
  FROM EMPLOYEES
...
SELECT * 
  FROM EMPLOYEES
  WHERE PHONE_NUMBER LIKE '515' || '%';   -- =를 사용하지않음
...
SELECT *
 ...
 SELECT *
   FROM EMPLOYEES
 ORDER BY DEPARTMENT_ID, SALARY DESC;  --함께 해석이아니라, 따로 귾어서 해석한다.
