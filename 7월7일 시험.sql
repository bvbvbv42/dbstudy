...
CREATE TABLE CUST_T(
     ...
     , CONSTRAINT FK_CUST  FOREIGN KEY(BANK_CODE) REFERENCES BANK_T(BANK_CODE) ON DELETE CASCADE
);
...
CREATE SEQUENCE CUST_SEQ
     ...
     CACHE 50
     ORDER;
INSERT INTO CUST_T(CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE) VALUES(CUST_SEQ.NEXTVAL, '고길동', '01011111111', TO_DATE('95/10/05'), 'BANK003');
INSERT INTO CUST_T(CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE) VALUES(CUST_SEQ.NEXTVAL, '조수진', '01022222222', TO_DATE('98/12/17'), 'BANK002');
INSERT INTO CUST_T(CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE) VALUES(CUST_SEQ.NEXTVAL, '최보미', '01033333333', TO_DATE('02/03/05'), 'BANK002');
INSERT INTO CUST_T(CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE) VALUES(CUST_SEQ.NEXTVAL, '유서은', '01044444444', TO_DATE('05/05/08'), 'BANK003');
COMMIT;
...
SELECT * FROM USER_IND_COLUMNS WHERE TABLE_NAME = 'CUST_T';

-- 6. 다음 뷰를 생성하시오.(5점)
--    1) 뷰이름 : V_SEOUL_BANK
--    2) 쿼리문 : '서울은행'의 BANK_CODE, BANK_NAME, BANK_TEL 정보를 조회할 수 있는 SELECT문
DROP VIEW V_SEOUL_BANK;
CREATE VIEW V_SEOUL_BANK
AS (SELECT BANK_CODE, BANK_NAME, BANK_TEL
      FROM BANK_T);
...
-- CUST_NO  CUST_NAME  CUST_TEL     CUST_BORN  BANK_CODE
-- 100      고길동     01011111111  95/10/05   BANK003
SELECT C.CUST_NO, C.CUST_NAME, C.CUST_TEL, C.CUST_BORN , B.BANK_CODE
