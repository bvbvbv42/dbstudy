/*
    서브쿼리(sub query)
    1. 메인쿼리에 포함되는 하위 쿼리를 서브 쿼리라고 한다
...
/*
    ...
    2. 단일  행 서브쿼리인 경우
        ...
        2) 통계 함수를 사용한 경우 SELECT COUNT(*) FROM EMPLOYEES
    3. 단일 행 서브쿼리 연산자
        =, !=. >, >=, <, <=
...
SELECT *
 ...
 WHERE JOB_ID = (SELECT JOB_ID 
                  ...
                  -- EMPLOYEE_ID가 101인 JOB_ID를 조회
                  -- EMPLOYEE_ID가 101인사람과 동일한 JOB_ID에서 생성되는 EMPLOYEES 조회
...
WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
                         FROM DEPARTMENTS
                        WHERE DEPARTMENT_NAME = 'IT');  -- 서브쿼리의 DEPARTMENT_NAME 칼럼은 중복이 있을 수 있으므로
...
-- 부서번호가 50인 부서에 근무하는 사원번호, 사원이름, 부서이름 조회하기 (비상관)
SELECT EMPLOYEE_ID
     , FIRST_NAME
     , LAST_NAME
 ...
 -- 부서번호가 50인 부서에 근무하는 사원번호, 사원이름, 부서이름 조회하기 (상관,D.DEPARTMENT_ID = E.DEPARTMENT_ID)
