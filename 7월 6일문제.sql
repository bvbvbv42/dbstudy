...
SELECT FIRST_NAME || ' ' || LAST_NAME AS FULLNAME
     , JOB_ID
  FROM EMPLOYEES
 WHERE JOB_ID LIKE CONCAT(CONCAT('IT','_'), 'PROG') OR JOB_ID LIKE CONCAT(CONCAT('ST','_'), 'MAN');

-- 6. EMPLOYEES 테이블에서 HIRE_DATE가 2005년도인 사원을 조회하시오.
SELECT FIRST_NAME || ' ' || LAST_NAME AS FULLNAME
     , HIRE_DATE
  FROM EMPLOYEES
 WHERE HIRE_DATE LIKE CONCAT('05', '%');
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS FULLNAME
     , NVL(SALARY * COMMISSION_PCT, 0) AS COMMISSION
  FROM EMPLOYEES;

-- 9. EMPLOYEES 테이블에서 FIRST_NAME이 'J'로 시작하는 사원을 조회하시오.
 SELECT FIRST_NAME 
  FROM EMPLOYEES
  WHERE FIRST_NAME LIKE CONCAT('J','%');
   

-- 10. EMPLOYEES 테이블에서 JOB_ID가 'MAN'으로 끝나는 사원들의 MANAGER_ID를 중복을 제거하여 조회하시오.
SELECT JOB_ID
  FROM EMPLOYEES
 WHERE JOB_ID LIKE CONCAT(CONCAT(CONCAT('N','%')'A','%')'M','%');


-- 11. EMPLOYEES 테이블에서 전체 사원을 DEPARTMENT_ID의 오름차순으로 조회하되, 
--동일한 DEPARTMENT_ID 내에서는 HIRE_DATE의 오름차순으로 조회하시오.
