...
SELECT AVG(COMMISSION_PCT) AS 커미션퍼센평균   --모든함수는 NULL값은 제외
    FROM EMPLOYEES;
...
-- 6. 전체 사원 수 조회하기
-- 1) NOT NULL이 확실한 칼럼으로 갯수를 구한다. EX)PK

SELECT COUNT(EMPLOYEE_ID) AS 전체사원수
  FROM EMPLOYEES;
  
  SELECT COUNT(FIRST_NAME) AS 전체사원수
  FROM EMPLOYEES;

  SELECT COUNT(COMMISSION_PCT) AS 전체사원수
  FROM EMPLOYEES;  
-- 2) 모든 칼럼으로 갯수를 구한다.

SELECT COUNT(*) AS 전체사원수 --전체 행 숫자 세기
  FROM EMPLOYEES;
