SELECT 1 + 1 FROM DUAL;                  --FROM을 쓰지않아도 되는데 쓰라고 나올때 사용
...
*/
SELECT TO_CHAR(1234)
     , TO_CHAR(1234, '999999')          --'  1234' , --999999:여섯자리로 나타내라, --9999999:일곱자리로 나타내라.
...
SELECT SYSDATE         --'23/07/04'
     , SYSTIMESTAMP    --'23/07/04 14:47:09.926000000 +09:00'
  FROM DUAL;
  
-- 현재 날짜와 시간 - 형식 지정
SELECT TO_CHAR(SYSDATE,       'YYYY-MM-DD AM HH:MI:SS')
     , TO_CHAR(SYSTIMESTAMP , 'YYYY-MM-DD HH:MI:SS')     --디폴트로 사용하면 날짜만 쓴다, BUT 시간도 함께 사용할 수 있는 함수다.
  FROM DUAL;
...
SELECT * 
 ...
 SELECT *
  ...
  --DT1이 '23/07/04'인 데이터를 조회하기
 SELECT *
   FROM EXAMPLE_T
  WHERE TO_DATE(DT1, 'YY/MM/DD') = TO_DATE('23/07/04', 'YY/MM/DD');
  
  --DT2가 '23/07/04'인 데이터를 조회하기
  SELECT *
   FROM EXAMPLE_T
  WHERE TO_TIMESTAMP(DT2, 'YY/MM/DD') = TO_TIMESTAMP('23/07/04', 'YY/MM/DD');